...
def add_true_node(tree, trajectory, index, parent_node_id, nary):
    for i in range(nary - 1, 0, -1):
        if index - i >= 0:
            ...
            node_id = tree.number_of_nodes()
            tree.add_node(node_id, x=[0] * 6, time=0, y=[-1] * 3, mask=0, mask2=0, type=-1)
            tree.add_edge(node_id, parent_node_id)

    sub_parent_node_id = tree.number_of_nodes()
    tree.add_node(sub_parent_node_id, x=[0] * 6, time=0, y=[-1] * 3, mask=0, mask2=0, type=-1)
    tree.add_edge(sub_parent_node_id, parent_node_id)

    if index - (nary - 1) > 0:
        add_true_node(tree, trajectory, index - (nary - 1), sub_parent_node_id, nary)
        tree.add_node(sub_parent_node_id, x=[0] * 6, time=0, y=trajectory[index - (nary - 1)]['labels'], mask=0,
                      mask2=0, type=-1)
...
def add_day_node(tree, trajectory, labels, index, nary):
    node_id = tree.number_of_nodes()
    tree.add_node(node_id, x=[0] * 6, time=0, y=labels[index], mask=0, mask2=1, type=0)
    if index > 0:  # recursion
        ...
        tree.add_node(fake_node_id, x=[0] * 6, time=0, y=[-1] * 3, mask=0, mask2=0, type=-1)
