...
if __name__ == '__main__':
    ...
    save_dir = os.path.join(args.save_path, f"{args.dataset}_{args.time_slots}-slots_{logging_time}")
    ...
    logging.basicConfig(level=logging.INFO,
                        ...
                        datefmt='%Y-%m-%d %H:%M:%S',
                        filename=os.path.join(save_dir, f'{args.dataset}_{args.time_slots}.log'))
    ...
    train_dataset = TrajectoryTrainDataset(train_df, map_set, args.time_slots)
    val_dataset = TrajectoryValDataset(val_df, map_set, args.time_slots)
    test_dataset = TrajectoryTestDataset(test_df, map_set, args.time_slots)
    ...
    TreeLSTM_model = TreeLSTM(h_size=args.h_size, nary=args.time_slots + 1,
    ...
    for epoch in range(args.epochs):
        ...
        for b_idx, batch in tqdm(enumerate(train_dataloader), total=len(train_dataloader), desc="Training"):
            ...
            for trajectory, label in batch:
                mobility_tree = construct_MobilityTree(trajectory, label, args.time_slots + 1, args.plot_tree)
